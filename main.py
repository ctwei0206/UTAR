...
uploaded_image = st.file_uploader("Choose a sleeping position image",
                                  type=["jpg", "jpeg", "png"])
...
if uploaded_image:
    ...
    st.image(img,
             caption="Uploaded Baby Sleeping Position",
             use_column_width=True)

    # Generate description from image (make sure this part is compatible with the Gemini API)
    img_description = baby_sleeping_model.generate_content(
...
baby_age = st.number_input("Baby's age (in months):",
                           min_value=0,
                           max_value=48,
                           step=1)

# Input for sleeping time
baby_sleeping_time = st.number_input("Sleeping Time (in hours):",
                                     min_value=0,
                                     max_value=24,
                                     step=1)
...
if st.button("Check Sleep"):
    if (uploaded_image is not None) and (baby_age > 0):
        try:
            ...
            result = baby_sleeping(baby_age, baby_sleeping_time,
                                   baby_pattern_description)
